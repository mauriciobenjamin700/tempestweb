 ...
 * event. A `reload` event means a rebuild completed server-side, so the tab is
 * reloaded to pick it up: `location.reload()` re-fetches index.html and every
 * (cache-busted) asset. The browser's EventSource reconnects automatically if the
 * dev server restarts, so a server bounce just resumes the stream.
 *
