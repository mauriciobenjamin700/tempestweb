...
!!! info "Dentro do subset hoje"
    ...
    - **Métodos stdlib:** string/list (`.upper`/`.lower`/`.strip`/`.startswith`/
      ...
      (`app.replace`, `native.storage.get`, `ctrl.forward`) passam intactos — use
      subscrito `d[k]` no lugar de `dict.get`.
...
!!! check "O kwarg de widget é validado no build"
    O Modo C não tem Python em runtime: o builder gerado desestrutura o objeto
    que recebe e **ignora** chave que não nomeia. Por isso o compilador confere
    cada chamada de modelo do core contra os campos reais e falha com
    `arquivo:linha` — `Container(children=[...])` viraria uma caixa vazia aqui e
    `ValidationError` nos Modos A/B. O slot de filho usa **o nome do core**:
...
!!! check "Componentes estruturais estão portados"
    ...
    - **divulgação e seleção de painel:** `Accordion` (o corpo só existe na
      árvore quando `open`, então fechar é *remove*, não esconder) e `Tabs` (a
    ...
    core real — **360 casos** — então drift de composição ou de estilo falha no
    ...
    **pares `__keyed`**: o mesmo componente construído
    ...
    verdade). O comparador também passou a olhar a chave de cada **descendente** —
    antes só comparava forma e estilo, e por isso um port com chave literal
    atravessou um release inteiro.
...
!!! warning "Ainda fora do subset — e agora falha no build"
    **Método de widget do core** (`form.validate(values)`, e qualquer outro): o
    ...
    (`DataTable`, `Table`, `BarChart`/`LineChart`, `DetectionOverlay`,
    `ResultView`, `Calendar`/`Clock`, os pickers de mídia e de formulário, e o
    `CollapsingAppBar`, que depende do scroll): a *forma da árvore* deles depende
    dos dados recebidos — uma linha de células por registro, uma barra por dado —
    então não há composição fixa para portar. Percorrer uma lista plana de
    rótulos não é isso: `Tabs` e `Accordion` são composição fixa e estão
    servidos, como os estruturais acima
