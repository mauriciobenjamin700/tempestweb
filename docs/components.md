...
No total, `tempestweb.components` expõe **67 componentes** (+ 10 helpers): scaffolds
de layout, app bars, navegação, `Card`, `ListTile`, inputs, feedback
(`Alert`/`Banner`/`Badge`), tabelas (`Table`/`DataTable`) e **gráficos**
(`BarChart`/`LineChart`).
