...
```bash
tempestweb dev --mode wasm    --path examples/device-panel   # Python in the browser (Pyodide)
tempestweb dev --mode server  --path examples/device-panel   # Python on the server (FastAPI + WS)
```
