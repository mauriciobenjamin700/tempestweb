...
```bash
tempestweb dev --mode wasm     --path examples/fetch   # Python in the browser (Pyodide)
tempestweb dev --mode server   --path examples/fetch   # Python on the server (FastAPI + WS)
```
