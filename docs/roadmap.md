...
| S2 | **Limites & anti-DoS:** `max_connections` (teto WS+SSE, recusa `1013`/`503`) + `max_message_bytes` (POST SSE `413`). Pendente: timeout de sessão ociosa, teto de mensagem no WS, rate limiting por IP (usar reverse-proxy por ora) | 🔶 (v0.42.0 — cap + payload feitos; idle/rate = follow-up) |
