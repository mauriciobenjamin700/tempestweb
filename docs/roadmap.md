...
| R4 | **storage (codec):** compressão opcional, **medida antes de decidir** — e a medição mudou a resposta: o IndexedDB já comprime (catálogo de 977 KB chega ao disco como 222 KB sozinho), então o codec economiza 45–65% do que **sobra**, custando +12 ms/leitura e +76 ms/escrita por MB num aparelho fraco. Default `json`, opt-in por `native.storage.configure(codec=...)` — não na fila de mutações, que a própria issue punha fora de escopo. Decodificar está sempre ligado, então ligar/desligar não apaga cache de ninguém. Origem: `src/utils/compressed-storage.ts` | ✅ (v0.117.0 — #180) |
