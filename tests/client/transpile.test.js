...
test("every ported component matches the core build (order-agnostic)", () => {
  ...
  const noop = () => {};
  const casesFor = (theme) => ({
    hstack_default: HStack({ children: [child()], theme }),
    hstack_lg_between: HStack({ gap: "lg", justify: "space-between", theme }),
    hstack_float: HStack({ gap: 8.0, theme }),
    vstack_sm: VStack({ children: [child()], gap: "sm", theme }),
    vstack_start: VStack({ children: [child()], align: "start", theme }),
    card_default: Card({ children: [child()], theme }),
    card_filled_primary: Card({ children: [child()], variant: "filled", colorScheme: "primary", theme }),
    card_outlined_error_flat: Card({
      children: [child()],
      variant: "outlined",
      colorScheme: "error",
      elevation: 0, theme }),
    card_elevated_level_4: Card({ children: [child()], elevation: 4, theme }),
    card_steps: Card({
      children: [child()],
      paddingStep: "lg",
      radiusStep: "xl",
      gapStep: "none", theme }),
    divider_default: Divider({ theme }),
    divider_token_thickness: Divider({ thickness: "xs", theme }),
    divider_tinted: Divider({ colorScheme: "primary", theme }),
    chip_static: Chip({ label: "tag", theme }),
    chip_selected: Chip({ label: "tag", selected: true, theme }),
    chip_clickable_lg_success: Chip({
      label: "tag",
      onClick: noop,
      size: "lg",
      colorScheme: "success", theme }),
    segmented_default: SegmentedControl({ options: ["a", "b"], onSelect: noop, theme }),
    ...
    appbar_title_only: AppBar({ title: "Home", theme }),
    ...
    appbar_outlined_primary_level_2: AppBar({
      ...
      colorScheme: "primary",
      elevation: 2, theme }),
    radio_default: RadioGroup({ options: ["a", "b"], onSelect: noop, theme }),
    radio_second_sm_warning: RadioGroup({
      options: ["a", "b"],
      selected: 1,
      onSelect: noop,
      size: "sm",
      colorScheme: "warning", theme }),
    scaffold_body_only: Scaffold({ body: child(), theme }),
    scaffold_full: Scaffold({
      appBar: AppBar({ title: "Home" }),
      body: child(),
      bottomBar: Divider(), theme }),
    scaffold_scroll: Scaffold({ body: child(), scroll: true, theme }),
    scaffold_empty: Scaffold({ theme }),
    surface_default: Surface({ child: child(), theme }),
    surface_filled_primary: Surface({ child: child(), variant: "filled", colorScheme: "primary", theme }),
    surface_outlined_error_flat: Surface({
      child: child(),
      variant: "outlined",
      colorScheme: "error",
      elevation: 0, theme }),
    surface_radius_lg: Surface({ child: child(), radiusStep: "lg", theme }),
    surface_empty: Surface({ theme }),
    styled_container_default: StyledContainer({ child: child(), theme }),
    styled_container_step_lg: StyledContainer({ child: child(), padding: "lg", theme }),
    styled_container_float: StyledContainer({ child: child(), padding: 6.0, theme }),
    grid_three_in_two: Grid({ children: [child(), child(), child()], theme }),
    grid_full_rows: Grid({ children: [child(), child(), child(), child()], theme }),
    grid_single_column: Grid({ children: [child(), child()], columns: 1, theme }),
    grid_four_in_three_token_gap: Grid({
      children: [child(), child(), child(), child()],
      columns: 3,
      gap: "md", theme }),
    grid_empty: Grid({ theme }),
    sidebar_default: Sidebar({ children: [child()], theme }),
    sidebar_filled_primary_wide: Sidebar({
      children: [child()],
      width: 320.0,
      variant: "filled",
      colorScheme: "primary", theme }),
    sidebar_outlined_level_3: Sidebar({ children: [child()], variant: "outlined", elevation: 3, theme }),
    drawer_closed: Drawer({ children: [child()], theme }),
    drawer_open: Drawer({ open: true, children: [child()], theme }),
    drawer_open_filled_secondary: Drawer({
      ...
      variant: "filled",
      colorScheme: "secondary", theme }),
    burger_default: Burger({ onClick: noop, theme }),
    burger_solid_primary_lg: Burger({
      onClick: noop,
      variant: "solid",
      colorScheme: "primary",
      size: "lg", theme }),
    header_title_only: Header({ title: "Reports", theme }),
    header_with_subtitle: Header({ title: "Reports", subtitle: "last 30 days", theme }),
    header_tinted: Header({ title: "Reports", colorScheme: "primary", theme }),
    header_neutral_scheme: Header({ title: "Reports", colorScheme: "neutral", theme }),
    footer_default: Footer({ children: [child()], theme }),
    footer_filled_primary: Footer({ children: [child()], variant: "filled", colorScheme: "primary", theme }),
    footer_outlined_flat: Footer({ children: [child()], variant: "outlined", elevation: 0, theme }),
    navbar_first_active: NavBar({ items: ["a", "b", "c"], onSelect: noop, theme }),
    navbar_second_active_lg: NavBar({
      items: ["a", "b", "c"],
      active: 1,
      onSelect: noop,
      size: "lg", theme }),
    navbar_secondary_scheme: NavBar({ items: ["a", "b"], onSelect: noop, colorScheme: "secondary", theme }),
    breadcrumb_presentational: Breadcrumb({ items: ["home", "docs", "ui"], theme }),
    breadcrumb_navigable: Breadcrumb({ items: ["home", "docs", "ui"], onSelect: noop, theme }),
    breadcrumb_single: Breadcrumb({ items: ["home"], onSelect: noop, theme }),
    breadcrumb_custom_separator: Breadcrumb({ items: ["a", "b"], separator: "›", theme }),
    listtile_title_only: ListTile({ title: "Maria", theme }),
    listtile_with_subtitle: ListTile({ title: "Maria", subtitle: "admin", theme }),
    listtile_with_slots: ListTile({
      title: "Maria",
      subtitle: "admin",
      leading: Avatar({ initials: "MB", key: "lead" }),
      trailing: Button({ label: "→", onClick: noop, key: "go" }), theme }),
    listtile_tinted: ListTile({ title: "Maria", colorScheme: "primary", theme }),
    listtile_neutral_scheme: ListTile({ title: "Maria", colorScheme: "neutral", theme }),
    avatar_default: Avatar({ initials: "MB", theme }),
    avatar_large_secondary: Avatar({ initials: "MB", size: 64.0, colorScheme: "secondary", theme }),
    avatar_neutral: Avatar({ initials: "MB", colorScheme: "neutral", theme }),
    avatar_unknown_scheme: Avatar({ initials: "MB", colorScheme: "brand", theme }),
    tag_default: Tag({ label: "python", theme }),
    tag_lg_success: Tag({ label: "python", size: "lg", colorScheme: "success", theme }),
    rating_presentational: Rating({ value: 3, theme }),
    rating_interactive: Rating({ value: 2, onRate: noop, theme }),
    rating_three_stars: Rating({ value: 1, maxStars: 3, colorScheme: "warning", theme }),
    stepper_default: Stepper({ onChange: noop, theme }),
    stepper_bounded: Stepper({ value: 5, step: 2, minValue: 0, maxValue: 10, onChange: noop, theme }),
    searchbar_empty: SearchBar({ onChange: noop, theme }),
    searchbar_with_value_and_clear: SearchBar({ value: "cat", onChange: noop, onClear: noop, theme }),
    searchbar_empty_with_clear: SearchBar({ onChange: noop, onClear: noop, theme }),
    searchbar_outline_sm_primary: SearchBar({
      onChange: noop,
      fieldVariant: "outline",
      size: "sm",
      colorScheme: "primary", theme }),
    banner_default: Banner({ message: "saved", theme }),
    banner_success_tone: Banner({ message: "saved", tone: "success", theme }),
    banner_unknown_tone: Banner({ message: "saved", tone: "fuchsia", theme }),
    banner_solid_scheme: Banner({ message: "saved", variant: "solid", colorScheme: "error", theme }),
    banner_left_accent_with_action: Banner({
      message: "saved",
      variant: "left_accent",
      action: Button({ label: "undo", onClick: noop, key: "undo" }), theme }),
    alert_title_only: Alert({ title: "Heads up", theme }),
    alert_body_and_glyph: Alert({ title: "Heads up", body: "check it", glyph: "!", theme }),
    alert_left_accent_error: Alert({
      title: "Heads up",
      variant: "left_accent",
      colorScheme: "error", theme }),
    alert_top_accent_with_dismiss: Alert({
      title: "Heads up",
      variant: "top_accent",
      colorScheme: "success",
      dismiss: Button({ label: "x", onClick: noop, key: "close" }), theme }),
    badge_default: Badge({ label: "3", theme }),
    badge_subtle_info_md: Badge({ label: "3", variant: "subtle", colorScheme: "info", size: "md", theme }),
    badge_outline_warning_lg: Badge({
      label: "NEW",
      variant: "outline",
      colorScheme: "warning",
      size: "lg", theme }),
    badge_success_tone: Badge({ label: "3", tone: "success", theme }),
    badge_unknown_tone: Badge({ label: "3", tone: "fuchsia", theme }),
    emptystate_default: EmptyState({ title: "Nothing here", theme }),
    emptystate_full: EmptyState({
      title: "Nothing here",
      subtitle: "add the first one",
      glyph: "◍",
      action: Button({ label: "add", onClick: noop, key: "add" }), theme }),
    stat_plain: Stat({ label: "revenue", value: "R$ 1.2M", theme }),
    stat_delta_up: Stat({ label: "revenue", value: "R$ 1.2M", delta: "+12%", theme }),
    stat_delta_down: Stat({
      label: "revenue",
      value: "R$ 1.2M",
      delta: "-3%",
      deltaUp: false, theme }),
    progress_stepper_first: ProgressStepper({ steps: ["a", "b", "c"], theme }),
    progress_stepper_second: ProgressStepper({ steps: ["a", "b", "c"], current: 1, theme }),
    progress_stepper_single: ProgressStepper({ steps: ["a"], theme }),
    progress_stepper_secondary: ProgressStepper({
      steps: ["a", "b"],
      current: 1,
      colorScheme: "secondary", theme }),
    metric_card_plain: MetricCard({ label: "users", value: "1.2k", theme }),
    metric_card_delta: MetricCard({ label: "users", value: "1.2k", delta: "+8%", theme }),
    metric_card_trailing: MetricCard({
      label: "users",
      value: "1.2k",
      trailing: Text({ content: "~", key: "spark" }), theme }),
    metric_card_filled_primary: MetricCard({
      label: "users",
      value: "1.2k",
      variant: "filled",
      colorScheme: "primary", theme }),
    stat_card_default: StatCard({ label: "users", value: "1.2k", theme }),
    stat_card_delta_down: StatCard({ label: "users", value: "1.2k", delta: "-2%", deltaUp: false, theme }),
    confidence_badge_high: ConfidenceBadge({ confidence: 0.92, theme }),
    confidence_badge_mid: ConfidenceBadge({ confidence: 0.61, theme }),
    confidence_badge_low: ConfidenceBadge({ confidence: 0.2, theme }),
    confidence_badge_labelled: ConfidenceBadge({ confidence: 0.92, label: "cat", theme }),
    confidence_badge_custom_thresholds: ConfidenceBadge({ confidence: 0.61, high: 0.6, mid: 0.3, theme }),
    accordion_closed: Accordion({ title: "Details", onToggle: noop, theme }),
    ...
    accordion_open_elevated_error: Accordion({
      ...
      colorScheme: "error",
      onToggle: noop, theme }),
    tabs_default: Tabs({ tabs: ["a", "b"], onSelect: noop, theme }),
    tabs_second_lg: Tabs({ tabs: ["a", "b", "c"], active: 1, onSelect: noop, size: "lg", theme }),
    tabs_secondary_sm: Tabs({ tabs: ["a"], onSelect: noop, colorScheme: "secondary", size: "sm", theme }),
    tabs_empty: Tabs({ tabs: [], onSelect: noop, theme }),
    tabs_active_out_of_range: Tabs({ tabs: ["a", "b"], active: 7, onSelect: noop, theme }),
    email_input_default: EmailInput({ onChange: noop, theme }),
    ...
    email_input_filled_lg_unlabelled: EmailInput({
      ...
      onChange: noop, theme }),
    password_input_default: PasswordInput({ onChange: noop, theme }),
    password_input_flushed_sm_error: PasswordInput({
      ...
      size: "sm",
      onChange: noop, theme }),
    phone_input_default: PhoneInput({ onChange: noop, theme }),
    phone_input_value_filled: PhoneInput({
      value: "(11) 99999-1234",
      fieldVariant: "filled",
      onChange: noop, theme }),
    cpf_input_default: CPFInput({ onChange: noop, theme }),
    cpf_input_error_lg: CPFInput({
      value: "529.982.247-25",
      error: "CPF inválido",
      size: "lg",
      onChange: noop, theme }),
    cnpj_input_default: CNPJInput({ onChange: noop, theme }),
    cnpj_input_outline_error_scheme: CNPJInput({
      value: "11.222.333/0001-81",
      error: "CNPJ inválido",
      colorScheme: "error",
      onChange: noop, theme }),
    address_input_default: AddressInput({ onChange: noop, theme }),
    address_input_filled_values: AddressInput({
      ...
      fieldVariant: "filled",
      onChange: noop, theme }),
    address_input_unlabelled_sm: AddressInput({ label: "", size: "sm", onChange: noop, theme }),
    text_field_default: TextField({ onChange: noop, theme }),
    text_field_labelled_error: TextField({
      ...
      key: "name",
      onChange: noop, theme }),
    email_field_default: EmailField({ onChange: noop, theme }),
